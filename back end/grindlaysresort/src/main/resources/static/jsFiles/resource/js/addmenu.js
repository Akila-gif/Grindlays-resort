...
userPrivilage = HTTPRequestService("GET",'http://localhost:8080/privilege/Menu?user='+parent.window['logedUser']).data;
...
window.addEventListener("load", () => {
    ...
    refreshMenuTable(HTTPRequestService("GET",'http://localhost:8080/menu'),userPrivilage);
    //refreshInnerIngredientsDetailsTable(bedDetailsObject,MenuItemdetailsPrivilage);
    menuDropDownCreate(HTTPRequestService("GET",'http://localhost:8080/menuitems'));
    categoryDropDown(HTTPRequestService("GET",'http://localhost:8080/menu/category'));
});
...
refreshMenuTable = (dataList) => {
    //displaying data list for employee table
    const displayProperty = [
        { dataType: 'text', propertyName: "name" },
        { dataType: 'function', propertyName: menuCategoryDetailsTableFunction },
        { dataType: 'function', propertyName: MenuItemDetailsTableFunction },
        { dataType: 'text', propertyName: "per_amount_price" }
    ];
    ...
    fillDataIntoTable(MenuView, dataList, displayProperty, EditfunctionName, DeleteFunctionName, MoreFunctionName,deleteStatusFunction, true,userPrivilage);
...
const menuCategoryDetailsTableFunction = (element) => {
    return ('<p class="text">'+element.menuCategory.category+'</p>');
}
const MenuItemDetailsTableFunction = (element) => {
    var eleList = '<ol>';
    element.menuHasMenuItems.forEach((MenuItemDetails) => {
...
refreshInnerManuItemDetailsTable = (dataList) => {
    //displaying data list for employee table
    console.log(dataList);
    ItemSuggestionPrice = 0;
    const MenuIteminnerdisplayProperty = [
        { dataType: 'function', propertyName: MenuItemNameViewFunction},
...
let MenuItemNameViewFunction = (element) =>{
    return ('<p class="text">'+element.menuitem.itemName+'</p>');
}

let PriceViewFunction = (element) =>{
    return ('<p class="text">'+element.menuitem.unitPrice+'</p>');
}

let TotalIngredientPriceViewFunction = (element) =>{
    ItemSuggestionPrice+= element.menuitem.unitPrice * element.itemquentity;
    return ('<p class="text">'+element.menuitem.unitPrice * element.itemquentity+'</p>');
}
...
bedTypeAdd.addEventListener('click', () => {
    ...
    if (menuItemFormSelect.value!=="" && menuItemCountTxt.value!==""){
        let menuItemDeatailsDropDown = JSON.parse(menuItemFormSelect.value);

        let menuItemHasIngredientsViewDetails = {menuitem: menuItemDeatailsDropDown, itemquentity: parseFloat(menuItemCountTxt.value)};
        if (menuItemHasIngredientsViewDetails.menuitem.id === bedDetails.map(element => element.menuitem.id).find(element => element === menuItemHasIngredientsViewDetails.menuitem.id)){
            alertFunction("Ingredient Added","Do you want to add again ? <br><p type=\"button\" class=\"btn btn-outline-dark btn-sm mt-2\">Yes</p>","warning",()=>{
                ...
                let index = bedDetails.map(element => element.menuitem.id).findIndex(element => element === menuItemHasIngredientsViewDetails.menuitem.id);
                bedDetails[index].needquentity = parseInt(bedDetails[index].itemquentity) + parseInt(menuItemHasIngredientsViewDetails.itemquentity);
                bedDetails[index].headcount = parseInt(bedDetails[index].itemquentity) + parseInt(menuItemHasIngredientsViewDetails.itemquentity);
                refreshInnerManuItemDetailsTable(bedDetailsObject,MenuItemdetailsPrivilage);

            });
        }else {bedDetails.push(menuItemHasIngredientsViewDetails);bedDetailsObject.data = bedDetails;refreshInnerManuItemDetailsTable(bedDetailsObject,MenuItemdetailsPrivilage);}
        menuItemFormSelect.selectedIndex = 0;
        menuItemCountTxt.value = "";
    }
});
