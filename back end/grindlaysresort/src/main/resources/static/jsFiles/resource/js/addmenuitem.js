...
userPrivilage = HTTPRequestService("GET",'http://localhost:8080/privilege/MenuItem?user='+parent.window['logedUser']).data;
...
window.addEventListener("load", () => {
    ...
    refreshMenuItemTable(HTTPRequestService("GET",'http://localhost:8080/menuitems'),userPrivilage);
    //refreshInnerIngredientsDetailsTable(bedDetailsObject,MenuItemdetailsPrivilage);
    ingredientsDropDownCreate(HTTPRequestService("GET",'http://localhost:8080/ingredients'));
});
...
refreshMenuItemTable = (dataList) => {
    ...
    fillDataIntoTable(MenuItemView, dataList, displayProperty, EditfunctionName, DeleteFunctionName, MoreFunctionName,deleteStatusFunction, true,userPrivilage);
    $(document).ready(function () {
        $('#MenuItemView').DataTable();
        $('.dataTables_length').addClass('bs-select');

        MenuItemView_filter.classList.add('mb-3', 'pt-2');
        let searchInputDiv = MenuItemView_filter.children[0].children[0];
...
const ingredientsDetails = (element) => {
    var eleList = '<ol>';
    element.menuItemHasIngredients.forEach((ingredientDetails) => {
        console.log(ingredientDetails.ingredients.count);
        console.log(ingredientDetails.needquentity);
        const availablePortions = ingredientDetails.ingredients.count / ingredientDetails.needquentity;

        console.log(availablePortions);
        let bgClass;

        if (availablePortions >= 12) {
            bgClass = 'bg-success';
        } else if (availablePortions >= 7) {
            bgClass = 'bg-warning';
        } else {
            bgClass = 'bg-danger';
        }

        eleList += '<li class="text ' + bgClass + ' text-center rounded-pill m-1 p-1" style="list-style-type: none;">'
            + 'Ingredient : ' + ingredientDetails.ingredients.name + ' ' + ingredientDetails.needquentity + '</li>';
    });
    eleList += '</ol>';
    return eleList;
}
...
refreshInnerIngredientsDetailsTable = (dataList) => {
    //displaying data list for employee table
    ItemSuggestionPrice = 0;
    const MenuIteminnerdisplayProperty = [
        { dataType: 'function', propertyName: IngredientsNameViewFunction},
        { dataType: 'function', propertyName: IngredientscodeViewFunction },
        { dataType: 'function', propertyName: PriceViewFunction },
        { dataType: 'text', propertyName: "needquentity" },
        { dataType: 'function', propertyName: TotalIngredientPriceViewFunction }
    ];

    dataList.data.length === 0 ? IngredientsView.style.visibility = "hidden" : IngredientsView.style.visibility = "visible";
    // deleteStatusFunction - this parameter use for define delete button visibility in the table (if the delete status is true then the delete button will be visibility)
    fillDataIntoTable(IngredientsView, dataList, MenuIteminnerdisplayProperty, innerIngredientFormEditfunctionName,innerIngredientFormDeleteFunction, innerIngredientFormMoreFunctionName, innerIngredientFormdeleteStatusFunction ,true,MenuItemdetailsPrivilage);
    $(document).ready(function () {
    ...
    roomNameTxt.value = ItemSuggestionPrice;
}
...
const ingredientsDropDownCreate = (ingredientsList) =>{
    console.log(ingredientsList);
    dropDownCreate(ingredientsList,ingredientsFormSelect,'name');
}
const MenuItemTypeDropDown = (MenuItemTypeList) =>{
    dropDownCreate(MenuItemTypeList,MenuItemTypeFormSelect,'name');
}
const bedTypeDropDown = (bedTypeList) =>{
    dropDownCreate(bedTypeList,bedTypeFormSelect,'type');
}
const MenuItemStatusDropDown = (MenuItemStatusList) =>{
    dropDownCreate(MenuItemStatusList,MenuItemStatusFormSelect,'status');
}
...
let bedDetailsViewFunction = (element) =>{
    let MenuItemRequestData = HTTPRequestService("GET", 'http://localhost:8080/MenuItem/detailsbedcountinMenuItem?MenuItem_id=' + element.id).data;
    let mainDiv = document.createElement('div');
    MenuItemRequestData.forEach(elementOfBedDetails => {
...
let IngredientsNameViewFunction = (element) =>{
    return ('<p class="text">'+element.ingredients.name+'</p>');
}

let IngredientscodeViewFunction = (element) =>{
    return ('<p class="text">'+element.ingredients.ingredientscode+'</p>');
}

let PriceViewFunction = (element) =>{
    return ('<p class="text">'+element.ingredients.price+'</p>');
}

let TotalIngredientPriceViewFunction = (element) =>{
    ItemSuggestionPrice+= element.ingredients.price * element.needquentity;
    return ('<p class="text">'+element.ingredients.price*element.needquentity+'</p>');
}
...
bedTypeAdd.addEventListener('click', () => {
    ...
    if (ingredientsFormSelect.value!=="" && ingredientsCountTxt.value!==""){
        let IngredientDeatailsDropDown = JSON.parse(ingredientsFormSelect.value);

        let menuItemHasIngredientsViewDetails = {ingredients: IngredientDeatailsDropDown, needquentity: parseFloat(ingredientsCountTxt.value)};
        if (menuItemHasIngredientsViewDetails.ingredients.id === bedDetails.map(element => element.ingredients.id).find(element => element === menuItemHasIngredientsViewDetails.ingredients.id)){
            alertFunction("Ingredient Added","Do you want to add again ? <br><p type=\"button\" class=\"btn btn-outline-dark btn-sm mt-2\">Yes</p>","warning",()=>{
                ...
                let index = bedDetails.map(element => element.ingredients.id).findIndex(element => element === menuItemHasIngredientsViewDetails.ingredients.id);
                bedDetails[index].needquentity = parseInt(bedDetails[index].needquentity) + parseInt(menuItemHasIngredientsViewDetails.needquentity);
                bedDetails[index].headcount = parseInt(bedDetails[index].needquentity) + parseInt(menuItemHasIngredientsViewDetails.needquentity);
                refreshInnerIngredientsDetailsTable(bedDetailsObject,MenuItemdetailsPrivilage);

            });
        }else {bedDetails.push(menuItemHasIngredientsViewDetails);bedDetailsObject.data = bedDetails;refreshInnerIngredientsDetailsTable(bedDetailsObject,MenuItemdetailsPrivilage);}
        ingredientsFormSelect.selectedIndex = 0;
        ingredientsCountTxt.value = "";
    }
});
...
const valdationFeildList = [
    { id: 'itemNameTxt', type: 'text', validationStategy: 'nothing', requird: true},
...
MenuItemViewFeaturesTxt.addEventListener('keyup',(event)=>{
